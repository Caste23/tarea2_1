...
namespace tarea2_1.controller
{
    internal class countri
    {
        ...
        //hacemos un contructor vacio
        public countri()
        {

        }

        public async Task<List<countriesmodel>> GetCountries(string region)
        {
            try
            {
                ...
                using (HttpClient client = new HttpClient())
                {
                    ...
                    if (resp.IsSuccessStatusCode)
                    {
                        ...
                        countriesmodel temp = null;

                        foreach (var item in results)
                        {
                            temp = new countriesmodel();

                            var curTemp = new List<countriesmodel.Currencies>();
                            foreach (var current in item["currencies"].Values())
                            {
                                try
                                {
                                    curTemp.Add(new countriesmodel.Currencies()
                                    {
                            ...
                            temp.NameCountry = new countriesmodel.Name()
                            {
                            ...
                            temp.maps = new countriesmodel.Maps()
                            {
                            ...
                            temp.flags = new countriesmodel.Flags()
                            {
                            ...
                            try
                            {
                                temp.idd = new countriesmodel.Idd()
                                {
